...
# Controls:
#   q = quit, s = send single frame, a = toggle auto mode
...
def post_frame(url, frame, return_partial=True, quality=90, timeout=10):
    ...
    if not ok: raise RuntimeError("Failed to encode JPEG")
...
def draw_hud(frame, last_result: dict | None, auto: bool, locked: bool, verif: dict | None):
    ...
    def put(s): 
        nonlocal y; cv2.putText(frame, s, (10,y), cv2.FONT_HERSHEY_SIMPLEX, 0.6, (255,255,255), 2, cv2.LINE_AA); y += 22
    ...
    if last_result:
        ...
        put(f"stage={stage}  total={total}ms")
...
def draw_boxes(frame, result):
    ...
    for b in boxes:
        ...
        cv2.putText(frame, f'{cls}:{conf:.2f}', (x1, max(16,y1-8)), cv2.FONT_HERSHEY_SIMPLEX, 0.55, (0,255,0), 2, cv2.LINE_AA)
...
def main():
    ...
    ap.add_argument("--lock-thresh", type=float, default=0.70, help="YOLO title_box conf threshold [0-1]")
    ...
    def try_send(frame):
        ...
        try:
            ...
            print(json.dumps({
                ...
                "title_box_conf": (res.get("title_box") or {}).get("conf")
            }, ensure_ascii=False))
    ...
    def try_verify_async(scan_res):
        ...
        payload_nie = scan_res.get("bpom_number")
        payload_text = scan_res.get("title_text")
        try:
            verif_result = post_verify(args.verify_url, nie=payload_nie, text=payload_text, timeout=max(20, args.timeout))
    ...
    while True:
        ...
        # Auto-lock: conf>=threshold or got bpom_number
        if not locked and last_result:
            tb = last_result.get("title_box") or {}
            tb_conf = float(tb.get("conf") or 0.0)
            has_bpom = bool(last_result.get("bpom_number"))
            if has_bpom or tb_conf >= args.lock_thresh:
                locked = True
                auto = False
                # jalankan verify di thread terpisah
        ...
        if k == ord('q'): break
            ...
            locked = False
            verif_result = None
            # opsional: kirim 1x frame saat resume
            last_sent_ms = 0
